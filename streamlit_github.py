...
page = st.sidebar.radio(
    ...
    index=0  # default langsung ke Placement Prediction
)

# Helper: buat DataFrame sesuai fitur training
...
if page == "Placement Prediction":
    ...
    if submitted:
        input_df = build_input_df(
            ...
            experience_score=internships_completed + projects_completed
        )
        st.write("Input Data:", input_df)

        pred = placement_model.predict(input_df)
        st.success(f"Prediction: {'Placed' if pred=='Placed' else 'Not Placed'}")

        # Visualization 1
        exp_labels = ["Projects", "Internships", "Hackathons", "Certifications"]
        exp_values = [projects_completed, internships_completed, hackathons_participated, certifications_count]

        fig, ax = plt.subplots()
        ax.bar(exp_labels, exp_values, color=["orange","skyblue","green","purple"])
        ax.set_ylabel("Count")
        ax.set_title("Experience Profile")
        st.pyplot(fig)

        # Visualization 2
        acad_labels = ["CGPA","10th %","12th %","Attendance %"]
        acad_values = [cgpa, tenth_percentage, twelfth_percentage, attendance_percentage]

        skill_labels = ["Coding","Communication","Aptitude"]
        skill_values = [coding_skill_rating, communication_skill_rating, aptitude_skill_rating]

        fig, (ax1, ax2) = plt.subplots(1,2, figsize=(10,4))
        ax1.bar(acad_labels, acad_values, color="navy")
        ax1.set_title("Academic Profile")
        ax2.bar(skill_labels, skill_values, color="teal")
        ax2.set_title("Skill Ratings")
        st.pyplot(fig)
...
# About Page
    ...
    st.title("ℹ️ About")
    st.write("Aplikasi ini dibuat dengan Streamlit untuk UTS Model Deployment.")
    st.write("2802403031 - Darren Ritz Junaidi")
